...
from all.crypto_bot.handlers.default_buttons import global_menu, global_menu_reply
from all.crypto_bot.services.event_playground import event_service
from all.crypto_bot.states.tier_state import WalletState
...
async def delete_wallet(callback: types.CallbackQuery, state: FSMContext):
    if callback.data.split("_")[-1] == "USD":
        await callback.message.delete()
        await callback.message.answer("Это стандартный кошелек, это нельзя удалять!")
